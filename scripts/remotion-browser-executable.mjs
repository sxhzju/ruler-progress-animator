...
const resolveRemotionVersion = () => {
  const remotionPackagePath = resolve(
    process.cwd(),
    "node_modules",
    "remotion",
    "package.json"
  ...
  if (version.length === 0) {
    throw new Error(
      `[create-vibe-motion] invalid remotion version in ${remotionPackagePath}.`
    );
  }
...
const runRemotionBrowserEnsure = ({ log }) => {
  ...
  if (result.error) {
    throw new Error(
      `[create-vibe-motion] failed to execute Remotion browser ensure: ${result.error.message}`
    );
  }

  if (typeof result.status === "number" && result.status !== 0) {
    throw new Error(
      `[create-vibe-motion] Remotion browser ensure exited with status ${result.status}.`
    );
  }
};
...
const removeStaleLockIfNeeded = ({ log }) => {
  ...
  try {
    ...
    if (!isPidRunning(pid) || lockAge > LOCK_TIMEOUT_MS) {
      rmSync(LOCK_DIR, { recursive: true, force: true });
      log(
        `Removed stale install lock (pid=${Number.isFinite(pid) ? pid : "unknown"}).`
      );
      return true;
    }
  } catch {
  ...
  while (true) {
    try {
      ...
      if (Date.now() - startedAt > LOCK_TIMEOUT_MS) {
        throw new Error(
          `[create-vibe-motion] timeout waiting for browser install lock: ${LOCK_DIR}`
        );
      }
...
const installToSharedCache = ({
  ...
  rmSync(resolve(dirname(destinationBrowserDirectory), getExecutableName(platformTag)), {
    force: true,
  });
...
export const ensureSharedBrowserExecutable = async ({
  ...
  try {
    ...
    const localBinaryPath = resolve(localBrowserDirectory, getExecutableName(platformTag));
